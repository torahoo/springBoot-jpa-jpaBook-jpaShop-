...
database : http://localhost:8082/login.jsp?jsessionid=1f1c03029d7b8e4091614fda37be508c
...
회원 기능 테스트 케이스 작성
(모든 테스트 케이스는 spring boot로 돌아가면 DB와 통신을 하기에(H2) 
H2 서버를 열어놓고 돌려야 한다.)
...
회원 기능 테스트 케이스 작성 2
 ...
 - 테스트 케이스를 완전히 격리된 환경으로 만들기
   - 메모리 DB -> spring boot에 있음
     - test폴더에 resource를 만들고 그안에 application.yml을 만듦
       -> test가 돌아갈때 test안의 application.yml이 우선적으로 돌아감
     - yml파일 안의 datasource url을 메모리로 바꾸기
     - jdbc:h2:mem:test
     - 물론 스프링 부트에서는 자동지원이기에 설정 주석처리 해도 잘 돌아감
     - 실 서버랑 테스트 서버랑 구분해서 돌아가게끔하는게 맞음
...
상품 도메인 개발
 ...
 - 상품 엔티티 개발
   - 비즈니스 로직 추가
     - 수량 증가
     - 수량 감소
       - 감소시에 0보다 작아지면 예외 터지게끔
       - 예외 클래스 생성 -> runtimeException 상속 후 오버라이드
     - quantity 수량 변경시 setter가 아닌 비즈니스로직 등을 통해
       변경해야 한다.

 - 상품 리포지토리 개발
 
 - 상품 서비스 개발
   - 테스트 케이스는 위의 멤버 테스트와 비슷하고 간단하기에
     수업에서는 생략
...
주문 도메인 개발
 ...
 - 주문, 주문상품 엔티티 개발
   - 주문 클래스
     - 생성 메서드
       - createOrder 메서드 작성
         - 생성시에 변경이 생기면 이 메서드만 변경 하면 됨
         - set을 통해 값을 받는 것이 아닌 new order시 마다
           createOrder를 불러와 값을 설정해야 함.
     - 비즈니스 로직
       - 주문 취소
         - Order 및 OrderItem 모두 cancel 메서드 추가
     - 조회 로직
       - 전체 주문 가격 조회
   - 주문 아이템 클래스
     - 생성 메서드
       - 아이템에 orderPrice를 그대로 쓰기에는 할인이나 쿠폰등이
         있으므로 따로 가져가는게 맞음.
       - 주문 클래스에서 생성 메서드 호출시 orderItem의
         생성 메서드를 통해 재고가 까지고 들어옴.
 ...
 - 주문 서비스 개발
   - OrderService
     - 주문
       ...
       - 주문상품 생성
       - 주문 생성
         - 주문상품 및 주문생성 시에 생성 메소드를 실행하는데
           이때, set메서드를 통해 값을 변화시킬 수 있는 가능성을
           막아둬야 한다. (기본 생성자를 protected로 만듦 or
           @NoArgsContructor(access = AccessLevel,PROTECTED) 설정)
       - 주문 저장
         - 주문 저장시에 persist는 cascade덕분에 orderRepository
           하나만 쓰는데 이때 cascade에 대해 정확히 이해가 가지
           않는다면 없이 쓰다가 나중에 이해가 갈때쯤 써도 무방하다.
           (cascade로 orderItem과 delivery는 orderRepository.save
           를 통해 함께 persist된다.)
     - 취소
       - 주문 엔티티 조회
       - 주문 취소
...
서비스 계층은 단순히 엔티티에 필요한 요청을 위힘하는 역할을 한다. 이처럼 엔티티가 
비즈니스 로직을 가지고 객체 지향의 특성을 적극 활용하는 것을 도메인 모델 패턴
(http://martinfowler.com/eaaCatalog/domainModel.html)이라 한다.
반대로 엔티티에는 비즈니스 로직이 거의 없고 서비스 계층에서 대부분의 비즈니스 로직을 
처리하는 것을 트랜젝션 스트립트 패턴
...
===2023-04-26===
 ...
 - OrderServiceTest
   - 테코 작성시에 Assertion을 쓰는데 강의와는 다르게
     expected, actual, message 순서로 assertEquals
     쓰는법이 좀 다르다.
